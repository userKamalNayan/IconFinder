...
class GetIconSetOfSelectedCategoryUseCase(private val iconRepository: IconRepository) {
    suspend fun invoke(identifier: String, count: Int, after: String, showOnlyFree: Boolean) =
        iconRepository.getIconSetOfCategory(identifier, count, after, showOnlyFree);
}